"""Ubuntu/Debian target — native, hardened systemd services (Ubuntu 22.04+ / Debian 12+).

Adapted from ``targets/fedora_fips.py`` — same shape, same systemd units, same on-host layout;
see that module's own docstring for the general design (it's reused here almost verbatim). The
distro deltas, all confined to this module:

* No FIPS fail-closed preflight. Stock Ubuntu/Debian ship no in-tree FIPS-140-validated OpenSSL
  module (the accredited path is Ubuntu Pro's ``fips-updates`` — a paid entitlement secdeploy
  can't enable for you), so this target WARNS loudly instead of aborting when the host isn't in
  FIPS mode — see ``deploy/ubuntu/fips-check.sh`` and docs/ubuntu.md#fips.
  Use ``fedora-fips`` instead when your accreditation boundary requires a fail-closed FIPS check.
* CA trust: installed to ``/usr/local/share/ca-certificates/*.crt`` (the ``.crt`` extension is
  REQUIRED on Debian-family hosts — ``update-ca-certificates`` ignores anything else) and applied
  with ``update-ca-certificates``, not fedora's ``/etc/pki/ca-trust/...`` + ``update-ca-trust``.
* Packages: ``apt-get`` in place of ``dnf`` for the one package-manager step this module actually
  runs itself (the secproxy runtime — nginx + certbot); everything else (node/npm, uv, a
  container runtime for the stacks) is an identical manual host prerequisite on both targets —
  see docs/ubuntu.md#2-runtimes.
* No SELinux steps to translate (fedora-fips has none — grep it) — AppArmor needs no
  configuration for these services either (the systemd sandboxing directives below are the same
  hardening boundary on both distros); see docs/ubuntu.md#hardening-applied.
* systemd units are reused directly from ``deploy/fedora-fips/systemd/`` (byte-identical — no
  SELinux/dnf-specific directives in them to begin with), rather than duplicated under
  ``deploy/ubuntu/``. Everything else (useradd flags, ``/opt``, ``/etc``, ``/var/lib``,
  systemd-resolved drop-in, secproxy logrotate) carries over unchanged — same shadow-utils
  ``useradd``, same FHS paths, same systemd tooling on both distros.

On-host layout (identical to fedora-fips)::
...
HONESTY NOTE: this target is dry-run/unit verified only — there is no live Ubuntu host in this
environment to validate a real deploy against. Treat it as pending first live validation (see
docs/ubuntu.md) until someone has run it for real.
"""
...
from .fedora_fips import STACK_NAMES  # the suite's only two stack components — see that module
...
# Debian-family CA trust store: unlike fedora's /etc/pki/ca-trust/source/anchors + update-ca-trust,
# update-ca-certificates ONLY picks up files here with a *.crt extension — see the trust-anchor
# step in _deploy_steps and its reverse in teardown_plan below.
CA_TRUST_DIR = Path("/usr/local/share/ca-certificates")
CA_ANCHOR_FILE = CA_TRUST_DIR / "secsuite-seccert-root.crt"
...
# guarded. nginx links the system OpenSSL (FIPS-validated on an Ubuntu Pro fips-updates host —
# see the FIPS advisory check; not on stock Ubuntu/Debian), which is why the suite standardized
# on it for the edge TLS termination (macOS runs the same nginx for its eval).
...
# systemd units are 100% distro-agnostic (no SELinux/dnf-specific directives in them at all —
# see the module docstring), so this target REUSES deploy/fedora-fips/systemd/ directly rather
# than duplicating the unit files under deploy/ubuntu/ — see docs/ubuntu.md for why (the unit
# dir is computed in _deploy_steps, same as fedora_fips.py computes its own preflight path).

PLAN = [
    "FIPS advisory check: warn (never abort) if the host isn't in FIPS mode / the OpenSSL FIPS "
    "provider isn't active — see docs/ubuntu.md#fips (unlike fedora-fips, which fails closed)",
    ...
    "Install hardened systemd units (reused from deploy/fedora-fips/systemd/) + secsuite.target; "
    "daemon-reload",
    "Enable + start secsuite.target (secdns + SecCert + SecLLM first, then SecRouter, then SecRecorder)",
    "Add the SecCert root to the host trust store (.crt extension + update-ca-certificates)",
...
def build(manifest: Manifest, work: Path, out: Path, root: Path,
    ...
    if "secrouter" not in without:
        ...
        if P.which("npm"):
            ...
            P.warn("npm not found — SecRouter must be built on the Ubuntu/Debian host (node>=24)")
    ...
    for name in ("seccert", "secrecorder", "secdns", "secllm", "secagent"):
        ...
        if P.which("uv") and (proj / "pyproject.toml").exists():
            ...
            P.warn(f"uv not found or {name} missing pyproject — build on the Ubuntu/Debian host")
...
def _secrecorder_hf_token_set() -> bool:
    ...
    ``_read_hf_token`` — same idea, different path: this target's env file IS the final one the
...
def _secproxy_setup_actions(
    ...
    same name. CA trust is automatic here (``update-ca-certificates`` runs whenever seccert is
    part of the deploy — see ``_deploy_steps``), so unlike macOS there's no separate trust-anchor
    action; only the resolver/HF_TOKEN/SecChat items can still be outstanding."""
    actions: list[str] = []
    if not resolver_configured:
        actions.append(
            "Point this host's resolver at SecDNS: "
            "<code>secdeploy deploy ubuntu --configure-resolver</code>."
        )
...
def _issue_secproxy_cert(topology, without: list[str] | None = None) -> list[tuple[list[str], str]]:
    ...
    BOOTSTRAP ORDERING — flagged, not fully solved this wave (identical mechanism to fedora-fips;
    see docs/fedora-fips.md#secproxy-edge-reverse-proxy for the general explanation, cross-
    referenced from docs/ubuntu.md rather than duplicated): certbot's HTTP-01 challenge is answered by
...
def _deploy_steps(manifest: Manifest, work: Path, root: Path,
    ...
    # Advisory (WARN-only) FIPS check — the fail-CLOSED fedora-fips preflight's opposite number;
    # see the module docstring for why. The systemd unit files themselves are reused directly
    # from deploy/fedora-fips/systemd/ — they carry no SELinux/dnf-specific directive to begin
    # with, so there is nothing distro-specific to duplicate under deploy/ubuntu/.
    fips_check = root / "deploy/ubuntu/fips-check.sh"
    unit_dir = root / "deploy/fedora-fips/systemd"
    steps: list[tuple[list[str], str]] = [
        (["bash", str(fips_check)], "FIPS advisory check (WARN-only, never aborts)"),
    ]
    # users + dirs
    for svc in services:
        user = f"secsuite-{svc}"
    ...
    # from secdeploy). PREFER a filled deploy/ubuntu/<svc>.env over the .example when one
    # exists — that's what `secdeploy configure`'s optional secret-seeding step writes for a
    # ubuntu resource (see configure.py's _write_env_seeds); a checkout no one has seeded still
    # falls back to the shipped .example. The .example itself is the SAME file fedora-fips ships
    # (deploy/fedora-fips/<svc>.env.example is behavior-identical — reused, not duplicated) EXCEPT
    # for the small set of components whose example text differs enough (doc references, the
    # FIPS-mode caveat) to warrant its own copy under deploy/ubuntu/ — see docs/ubuntu.md.
    for svc in services:
        if svc in ("secdns", "secllm", "secproxy"):
            continue
        filled = root / f"deploy/ubuntu/{svc}.env"
        example = root / f"deploy/ubuntu/{svc}.env.example"
    ...
    # own README). nginx links the system OpenSSL, which is FIPS-validated on an Ubuntu Pro
    # fips-updates host (not on stock Ubuntu/Debian — see the FIPS advisory check above) — the
    # same reasoning fedora-fips standardized on it for edge TLS termination with; macOS runs the
    # same nginx for a non-FIPS eval — see targets/macos.py. certbot is the ACME client that mints
    # secproxy's SAN cert from SecCert at deploy time (see _issue_secproxy_cert below) — nginx
    # runs no ACME client of its own. A native package under a dedicated hardened systemd unit
    # (not a podman container) was chosen deliberately, same reasoning as fedora-fips: secproxy's
    # manifest `kind` is "service" (like secdns), not "stack" (secsso/secchat's podman/compose
    # path — a wholly separate deploy_stacks() mechanism, see targets/common.py); podman would
    # also make binding :443/:80 with AmbientCapabilities=CAP_NET_BIND_SERVICE — the exact
    # mechanism secdns uses for :53 — far less direct across a container boundary. See
    # docs/fedora-fips.md#secproxy-edge-reverse-proxy (identical reasoning on ubuntu).
    if "secproxy" in services:
        steps.append((
            ["bash", "-c", "DEBIAN_FRONTEND=noninteractive apt-get install -y nginx certbot"],
            "install the secproxy runtime (nginx — system-OpenSSL TLS, FIPS-validated only on "
            "an Ubuntu Pro fips-updates host) + certbot",
        ))
    ...
    # trust anchor (only if SecCert is part of this deploy). NOTE the .crt extension — Debian's
    # update-ca-certificates only picks up files under CA_TRUST_DIR that end in .crt (fedora's
    # update-ca-trust has no such extension requirement on its own anchors dir).
    if "seccert" in services:
        steps.append((
            ["bash", "-c",
             "for i in $(seq 1 30); do curl -fsS http://127.0.0.1:47001/ca.crt "
             f"-o {CA_ANCHOR_FILE} && break; sleep 1; done && update-ca-certificates"],
            "add SecCert root to the host trust store (.crt extension required)",
        ))
    return steps
...
def deploy(
    ...
    users = users or []
    # native_services is a macOS-only knob (launchd install vs. print) — ubuntu, like
    # fedora-fips, is always systemd-native, so it's accepted for calling-convention parity with
    # macos.deploy() and otherwise ignored (same pattern as the macOS-only --tls/--configure-hosts
    # flags above).
    del native_services
    if hf_token or model_dir:
        P.warn("--hf-token/--model-dir are macOS-only — on ubuntu, set HF_TOKEN/"
               "WHISPER_MODEL/WHISPER_DIARIZE_MODEL directly in /etc/secsuite/secrecorder.env")
    if tls or configure_hosts or trust_ca:
        P.warn("--tls/--configure-hosts/--trust-ca are macOS-only (ubuntu gets TLS via "
    ...
    secproxy_cert_note = (
        ...
        "`nginx -s reload` — see docs/fedora-fips.md#secproxy-edge-reverse-proxy (identical "
        "mechanism on ubuntu)."
    )
    ...
    if dry_run:
        print(f"# ubuntu deploy plan — suite {manifest.suite} (run as root on the Ubuntu/Debian host)")
        if topology is not None:
            ...
            print("# addressing: writes secdns zone/env + peer env/ (also via "
                  f"`bundle ubuntu --resource {resource}`)")
    ...
    if platform.system() != "Linux":
        P.die(f"ubuntu deploy must run on the Ubuntu/Debian host (this is {platform.system()}). "
              "Use --dry-run to preview, or `secdeploy bundle ubuntu` to build a transfer bundle.")
    import os

    if os.geteuid() != 0:
        P.die("ubuntu deploy must run as root (systemd unit + trust-store install)")
    ...
    if addr_dir is not None:
        ...
        if "secllm" in services:
            ...
            # macOS only so far — ubuntu, like fedora-fips, runs the one static secllm.service
            # unit. Warn rather than silently under-deploying the declared pool (the generated
            ...
            if len(local_replicas) > 1:
                P.warn(f"secllm: {len(local_replicas)} instances declared on resource "
                       f"{resource!r}, but ubuntu automates only the first "
        ...
        if secagent_enabled and written.get("oidc"):
            P.log(f"SecRouter OIDC config fragment written → {written['oidc']} "
                  "(merge into security.oidc — see docs/fedora-fips.md, identical on ubuntu)")
    ...
    P.log("suite deployed — check `secdeploy status ubuntu`")


def status(manifest: Manifest, root: Path) -> None:
    if platform.system() != "Linux":
        P.warn("status is meaningful on the Ubuntu/Debian host; showing intended checks")
        for svc in SERVICES:
...
#
# Split for testability (see tests/test_teardown.py), the same shape targets/macos.py and
# targets/fedora_fips.py use:
#   _discover()     — probes the host, returns an UbuntuFound. Host-dependent (systemctl/
...
#                     discovery instead).
#   teardown_plan() — PURE: UbuntuFound + purge -> an ordered list of common.Step. No I/O, so
#                     tests exercise it directly with synthetic UbuntuFound values — the
...
# STACK_NAMES — the suite's only two `kind = "stack"` components (see suite.toml) — is imported
# from targets/fedora_fips.py (see the top of this module) rather than redefined here: teardown
# must stay 100% probe-driven (never read from a Manifest), and the hardcoded tuple itself is
# target-agnostic, so one definition serves both native-systemd targets.
...
class UbuntuFound:
    ...
    anchor_exists: bool             # CA_ANCHOR_FILE exists
...
def _discover(work: Path) -> UbuntuFound:
    ...
    return UbuntuFound(
        ...
        anchor_exists=CA_ANCHOR_FILE.is_file(),
        resolver_dropin_exists=RESOLVER_DROPIN.is_file(),
        stacks=stacks,
    )


def teardown_plan(found: UbuntuFound, purge: bool) -> list[common.Step]:
    ...
    Order matches docs/fedora-fips.md#teardown exactly (identical on ubuntu): services, users,
    code, config, data (--purge only — nothing under VAR is ever even MENTIONED without
    --purge), trust anchor, resolver, stacks, then a fixed "not removed" packages note.
    """
    ...
    # 6. trust anchor — rm before update-ca-certificates (so the refresh reflects its removal).
    if found.anchor_exists:
        steps.append(common.Step(
            f"remove the trust anchor {CA_ANCHOR_FILE}", ["rm", "-f", str(CA_ANCHOR_FILE)],
            "trust anchor",
        ))
        steps.append(common.Step(
            "refresh the system trust store", ["update-ca-certificates"], "trust anchor",
        ))
    ...
    steps.append(common.Step(
        ...
        "(nodejs/npm, uv, nginx, certbot, and a container runtime for the stacks — docker.io/"
        "docker-ce or podman) and the global npm package @earendil-works/pi-coding-agent — "
        "remove manually if you're sure nothing else on this host depends on them (an "
        "interactive `pi` session in particular needs the npm package)", None, "packages",
    ))
    return steps
...
def teardown(
    ...
    (which uses it as a best-effort /etc/resolver/<domain> hint) — ubuntu, like fedora-fips,
    needs no hint from it; every fact this function acts on comes from :func:`_discover`, never
    from that file. ``root`` is likewise unused here (this target's reverse actions are all
    absolute host paths — no repo asset needs locating) — kept for the same calling-convention
    parity ``status()`` already established (it accepts ``root`` too, and doesn't use it either)."""
    del root, topology  # unused — see docstring
    found = _discover(work)
    plan = teardown_plan(found, purge)
    print(f"# ubuntu teardown plan — suite {manifest.suite} — probed from THIS host, "
    ...
    if not any(s.command for s in plan):
        ...
        return
    if platform.system() != "Linux":
        P.die(f"ubuntu teardown must run on the Ubuntu/Debian host (this is {platform.system()}). "
              "Use --dry-run to preview.")
    import os

    if os.geteuid() != 0:
        P.die("ubuntu teardown must run as root (systemd units + /opt,/etc,/var removal)")

    print()
    n = sum(1 for s in plan if s.command)
    if not P.confirm(f"Proceed with ubuntu teardown ({n} command(s) above)?", assume_yes):
    ...
    P.log("ubuntu teardown complete")
...
def backup_native_steps(found: UbuntuFound, staging: Path) -> list[common.Step]:
...
def _backup_components_meta(found: UbuntuFound, stacks: list[tuple[str, Path]]) -> list[dict]:
...
def _require_root_linux(action: str) -> None:
    import os
    if platform.system() != "Linux":
        P.die(f"ubuntu {action} must run on the Ubuntu/Debian host (this is {platform.system()}). "
              "Use --dry-run to preview.")
    if os.geteuid() != 0:
        P.die(f"ubuntu {action} must run as root (touches /var/lib/secsuite + /etc/secsuite)")
...
def backup(
    ...
    print(f"# ubuntu backup plan — suite {manifest.suite} — one AES-256/CMS archive "
...
def restore(
    ...
    print(f"# ubuntu restore plan — suite {manifest.suite} — decrypt {archive_path.name}, "
    ...
    P.log("ubuntu restore complete — services restarted")
