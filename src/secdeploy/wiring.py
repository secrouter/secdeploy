...
def caddyfile_text(topology: Topology, without: list[str] | None = None) -> str:
    """Render the Caddyfile secproxy serves — the suite's one HTTPS front door (:443) for its
    FRONTED HTTP services (the ``fronted`` manifest flag; see :meth:`Topology.is_fronted`).

    A global-options block points Caddy's own ACME issuance at SecCert (this suite's internal
    CA) instead of a public one, via the ``acme_ca`` global option — SecCert's ACME directory
    is ``http://<seccert-fqdn>:<port>/acme/directory``, the same path convention the macOS
    ``--tls`` runbook's certbot invocation already uses (see ``targets/macos.py``'s
    ``_issue_secrecorder_cert``). Caddy trusts the SecCert root the same way every other client
    in the suite does — the system trust store, which the deploy installs separately
    (``--trust-ca`` on macOS, ``update-ca-trust`` on fedora-fips) — so there's nothing extra to
    configure for that here. Omitted entirely if SecCert isn't in this topology at all (e.g.
    dropped via ``--without``) — nothing to point ``acme_ca`` at.

    One site block per FRONTED, PLACED component instance, proxying straight to its real
    ``host:port`` — read directly from :meth:`Topology.instances` + the manifest's ``port``,
    the same direct-dial pattern :func:`secllm_endpoints`/:func:`host_port` use for SecRouter's
    own backend pool, so Caddy's upstream dial bypasses DNS/itself rather than looping back
    through the very front door it's serving. ``reverse_proxy`` handles WebSocket upgrades and
    sets the ``X-Forwarded-For``/``-Proto``/``-Host`` headers automatically — no extra
    directives needed for either. SecLLM (inference must dial direct, never hop through the
    proxy) and secdns (not HTTP) are never fronted, and secproxy never fronts itself; this is
    enforced explicitly here too, not just via the manifest's ``fronted`` flag. Deterministic
    manifest order, so the output is stable/testable; empty of site blocks (just the global
    options, if any) when nothing in this topology is both fronted and placed.
    """
    selected = topology.manifest.select(without)
    lines = [
        "# Caddyfile for secproxy — generated by secdeploy from the site topology",
        "# edit topology.toml, not this file",
        "",
    ]
    seccert = selected.get("seccert")
    if seccert is not None and seccert.port:
        acme_directory = f"http://{topology.fqdn('seccert')}:{seccert.port}/acme/directory"
        lines += [
            "{",
            f"\tacme_ca {acme_directory}",
            "\t# SecCert's root is trusted via the system trust store, which the deploy",
            "\t# installs separately (--trust-ca on macOS, update-ca-trust on fedora-fips) —",
            "\t# no extra trust config needed here.",
            "}",
            "",
        ]
    lines += [
        "# reverse_proxy below handles WebSocket upgrades and sets X-Forwarded-For/-Proto/-Host",
        "# automatically — no extra directives needed for either.",
        "",
    ]
    for name, c in selected.items():
        ...
        for instance_name, _res, addr in topology.instances(name):
            lines += [
                f"{topology.fqdn(instance_name)} {{",
                f"\treverse_proxy {addr}:{c.port}",
                "}",
                "",
            ]
    return "\n".join(lines).rstrip() + "\n"
...
def write_addressing(
    ...
    *,
    ...
    placed on ``resource``, also writes ``<out_dir>/Caddyfile`` (see :func:`caddyfile_text`) —
    secproxy's own reverse-proxy config for the topology's fronted services, resource-specific
    like the egress/OIDC files above (only the resource secproxy itself runs on needs it),
    unlike the suite-wide zone. Returns the written paths — note that these last three are
    ...
    if "secproxy" in placed:
        caddyfile_path = rdir / "Caddyfile"
        caddyfile_path.write_text(caddyfile_text(topology, without))
    ...
    if caddyfile_path is not None:
        result["caddyfile"] = caddyfile_path
    return result
